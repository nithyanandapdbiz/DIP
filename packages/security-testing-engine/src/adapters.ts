 ...
 * Azure DevOps, Jira, a cloud Security Hub and a SIEM differ in what they call a security
 * finding and where it lands — a Bug, an Issue, a Finding, an Event — not in the order the
 * engine publishes container -> requirements -> findings -> defects -> evidence ->
 * traceability. The adapter names the noun; it does not reorder, add or skip a step. The
 * orchestrator never learns which provider it received, and the conformance suite proves
 * two providers produce an identical publication sequence.
 */
...
export interface SecurityAdapter {
  ...
  publishRequirement(requirement: { id: string; source: string; control: string; satisfied: boolean }): { externalId: string };
...
function securityAdapterFor(provider: string, containerNoun: string, findingNoun: string): { adapter: SecurityAdapter; journal: AdapterJournal } {
  ...
  return {
    ...
    adapter: {
      ...
      publishRequirement: (r) => { journal.record('publishRequirement', `${r.source} ${r.control}: ${r.satisfied ? 'satisfied' : 'open'}`); return { externalId: nextId(`${provider}-req`) }; },
...
export function azureDevOpsSecurityAdapter(): { adapter: SecurityAdapter; journal: AdapterJournal } {
  return securityAdapterFor('azure-devops', 'Security Test Run', 'Bug');
}
 ...
 *
 * The orchestrator asks for a provider by name and receives an implementation. It does not
 * know which one it got, and it must not: an orchestrator that can name a provider can
 * branch on it.
 */
export class SecurityAdapterRegistry {
